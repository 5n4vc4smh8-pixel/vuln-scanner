...
impl ScanEngine {
    pub async fn new(cli: Cli) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let payloads = payloads::get_basic_payloads(cli.aggressive, cli.confirm_destructive);
        ...
        Ok(Self {
            ...
            payloads,
            results: Vec::new(),
            semaphore: Arc::new(Semaphore::new(cli.threads)),
    ...
    pub async fn scan(&mut self) -> Result<Vec<DetectedVuln>, Box<dyn std::error::Error>> {
        ...
        // Parâmetros descobertos pelo crawler por endpoint (ex.: /docs?id=123)
                ...
                let tasks: Vec<_> = payloads
                    ...
                    .map(|payload| {
                        ...
                        tokio::spawn(async move {
                            ...
                            Self::test_endpoint(&c, &u, &pl, aggressive, &ps).await
                        })
                    })
                    .collect();
                for t in tasks {
    ...
    async fn find_hidden_parameters(&self, url: &str, known_params: &[String]) -> Vec<String> {
        ...
        // Orçamento de 200 candidatos: mantém TODAS as palavras base fixas
        // (incl. executar_tarefa — caso real do usuário) e usa combos para preencher
        ...
        for endpoint in endpoints {
            for payload in &self.payloads {
                ...
                tasks.push(tokio::spawn(async move {
                    let result = Self::test_endpoint(&client, &url, &payload_clone, aggressive, &params).await;
                    drop(permit);
                    result
                }));
            }
    ...
    async fn test_endpoint(
        ...
        if payload.contains("' OR '1'='1") || payload.contains("'; DROP") || payload.contains("UNION SELECT") {
            for param in real_params {
                ...
                if let Ok(resp) = client.get(&test_url).await {
                    ...
                    let is_different = Self::response_differs_from_normal(client, url, &body).await;
                    ...
                    if payload_reached_server && is_different && (score >= 1 || sql_context) {
           ...
           payload.contains("boot.ini") || payload.contains("php://filter") || payload.contains("%00") {
            ...
            for param in real_params {
                ...
                if let Ok(resp) = client.get(&test_url).await {
                    ...
                    if score >= 1 && is_different && (!payload_reflected || reflected_in_error) {
                        let vuln = Vulnerability {
                            ...
                            if let Ok(location_str) = location.to_str() {
                                if location_str.contains("evil.com") {
                                    ...
                                    return Some(DetectedVuln {
                                        ...
                                        url: url.to_string(),
                                        parameter: Some(param.to_string()),
                                        evidence: format!("Redirect to: {}", location_str),
                                        sanitized_evidence: format!("Redirect to: {}", location_str),
                                    });
                                }
                            }
                        }
        ...
        if aggressive {
            ...
            if let Ok(resp) = client.get(&test_url).await {
                ...
                // Exige pelo menos 2 indicadores fortes OU "request blocked"/"access denied"
                // (evita "forbidden"/"access denied" genericos de sites comuns)
                if score >= 2 || body.to_lowercase().contains("request blocked") {
                    let vuln = Vulnerability {
    ...
    async fn response_differs_from_normal(client: &SecureHttpClient, url: &str, payload_body: &str) -> bool {
        let normal_url = if url.contains('?') {
            // remove so o nosso payload do query, mantendo os outros
            let (base, query) = url.split_once('?').unwrap();
            let clean_query: Vec<&str> = query.split('&')
                .filter(|part| !part.starts_with("file=") && !part.starts_with("path=") 
                              && !part.starts_with("cmd=") && !part.starts_with("id="))
                .collect();
            if clean_query.is_empty() {
                base.to_string()
            } else {
                format!("{}?{}", base, clean_query.join("&"))
            }
        } else {
            url.to_string()
        };
        // FIX v6: retry x2 com sleep 100ms — sob concorrência o alvo single-thread pode
        // falhar intermitentemente; tratar falha como "indefinido" e tentar de novo antes
        // de desistir (evita falso negativo em SQLi/OpenRedirect)
        for attempt in 0..2 {
            if attempt > 0 {
